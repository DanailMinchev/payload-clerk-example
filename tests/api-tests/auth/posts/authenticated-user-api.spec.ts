...
test.describe("authenticated user /api/posts api tests", () => {
  ...
  test("should not update all", async () => {
    ...
    await expect(
      allRolesPostsApiClient.findById(postOnePublished.id),
    ).resolves.toEqual(
      expect.objectContaining({
        id: postOnePublished.id,
        title: postOnePublished.title,
      }),
    );
    await expect(
      allRolesPostsApiClient.findById(postTwoDraft.id),
    ).resolves.toEqual(
      expect.objectContaining({
        id: postTwoDraft.id,
        title: postTwoDraft.title,
      }),
    );
  });
  ...
  test("should not delete all", async () => {
    ...
    await expect(
      allRolesPostsApiClient.findById(postOnePublished.id),
    ).resolves.toEqual(expect.objectContaining({ id: postOnePublished.id }));
    await expect(
      allRolesPostsApiClient.findById(postTwoDraft.id),
    ).resolves.toEqual(expect.objectContaining({ id: postTwoDraft.id }));
  });
